class Color {
  ...
  linear(slope, intercept) {
    var slope = slope == null ? 1 : slope;
    var intercept = intercept || 0;
...
class Solver {
  ...
  css(filters) {
    function fmt(idx, multiplier) {
      var multiplier = multiplier == null ? 1 : multiplier;
...
(function ($) {
  $(document).ready(function(){
    $('body').once('edge-icons').each(function () {
      var icons = $('[class^="sy-icon"]');
      icons.each(function() {
        var icon = $(this);
        var bgimage = icon.css('background-image');
        if (bgimage != 'none') {
          // MS Edge displaying icon as background image. color the icon.
          var rgbStr = icon.css('background-color');

          if (rgbStr.indexOf('rgb') === 0) {
            icon.css('background-color', 'transparent');
            var rgb = rgbStr.replace('rgb(', '').replace(')','' ).split(',').map(Number);
            const color = new Color(rgb[0], rgb[1], rgb[2]);
            const solver = new Solver(color);
            const result = solver.solve();

            icon.attr('style', result.filter + ' background-color: transparent;');
          }
        }
      });
